...
mkValidator VestingDatum{..} () context =
  let
    TxInfo{..} = scriptContextTxInfo context
    [signed1, signed2] = (`elem` txInfoSignatories) <$> [beneficiary1, beneficiary2]
    beforeDeadline = to deadline `contains` txInfoValidRange
  in
       traceIfTrue  "Redemption before deadline." (signed1 &&     beforeDeadline)
    || traceIfTrue  "Refund after deadline."      (signed2 && not beforeDeadline)
    || traceIfFalse "Invaid."                      False
