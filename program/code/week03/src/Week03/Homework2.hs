...
mkValidator beneficiary deadline _ context =
  let
    TxInfo{..} = scriptContextTxInfo context
    signed = beneficiary `elem` txInfoSignatories
    afterDeadline = from deadline `contains` txInfoValidRange
  in
       traceIfTrue  "Redemption at/after deadline." (signed && afterDeadline)
