...
instance ContractModel TSModel where
    ...
    nextState (Start w) = do
        ...
        (tsModel . at w) $= Just (TSState (Just 0) 0 0)
        wait 1

    nextState (SetPrice v w p) = do
        when (v == w) $
            (tsModel . ix v . tssPrice) $= Just p
        wait 1
                                  ...
                                  case m of
                                    Just t -> do
                                                let
                                                  n = t ^. tssToken
                                                  l = t ^. tssLovelace
                                                deposit w $ lovelaceValueOf l <> assetClassValue (tokens Map.! w) n <> nfts Map.! w
