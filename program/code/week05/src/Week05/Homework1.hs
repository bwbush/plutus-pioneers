...
mkPolicy pkh deadline ScriptContext{..} =
       traceIfFalse "owner signature missing" checkSig
    && traceIfFalse "deadline past"    checkDeadline
  where
    TxInfo{..} = scriptContextTxInfo
    checkSig = pkh `elem` txInfoSignatories
    checkDeadline = to deadline `contains` txInfoValidRange
